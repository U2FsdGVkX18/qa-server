...
public class RabbitMqListener {
    ...
    public void listenPayMessageDlxQueue(Message message, Channel channel, String msg) throws IOException {
        ...
        log.debug("日志->>>RabbitMqListener->>>message : [{}]", message);
        try {
            //做回调业务操作
            ...
            if (msg.startsWith("e")) {
                ...
                log.debug("日志->>>RabbitMqListener->>>消费者接收到消息 : [{}]", msg);
            } else {   //不满足条件则说明业务操作不通过,重新发送消息
                //首先把当前消息拒绝
                channel.basicReject(deliveryTag, false);
                log.debug("日志->>>RabbitMqListener->>>拒绝当前消息 : [{}]", msg);
                //然后重新发送消息
                ...
                log.debug("日志->>>RabbitMqListener->>>重新发送消息 : [{}]", msg);
            }
        } catch (IOException e) {
            log.debug("日志->>>RabbitMqListener->>>异常 : [{}]", e.getMessage());
        }
    }
}
